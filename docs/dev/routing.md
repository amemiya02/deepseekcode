...
- **`routing.Config`**（`internal/routing/classifier.go`）——`FlashModel` / `ProModel` / `StickyTurns`（升级后在 pro 上停留几步）。
- **`routing.Signals`**（同文件）——分类器的逐 turn 输入：`UserText`（用户原文）、`RepairErrorsLastTurn`（上一轮工具修复错误数）。
- **`routing.Decision`**（同文件）——裁决结果：`Model` / `Thinking` / `Effort`（`""`、`low`…`max`）/ `StickyLeft`（剩余粘滞步数）/ `Reason`（`sticky` / `repair_errors` / `hard_reasoning` / `mechanical` 四值之一）。
- **`Agent` 路由字段**（`internal/agent/agent.go`）——`AutoRoute`（turn 前路由总开关）、`EscalationModel`（pro 目标，同时是 turn 内升级的开关）、`lastRoute routing.Decision`（粘滞状态跨步携带）、`AutoClarify`（澄清前置闸开关）。
- **`escalationRepairThreshold`**（`agent.go` 常量）——`= 3`，turn 内升级的修复错误阈值。
...
每个 step（一次模型调用）开头，`Agent.runStep` 先调 `a.routeTurn(lastUserText, 0)`，返回值直接填进本次 `llm.Request` 的 `Model` / `Thinking` / `ReasoningEffort` 三个字段——生效范围就是这一步，下一步重新判定。`routeTurn` 把本次 `Decision` 存进 `a.lastRoute`，粘滞状态由此跨步传递（session 内存级，不持久化）。

开关与接线：`AutoRoute && EscalationModel != ""` 才走 `routing.Classify`，否则回退到旧路径（`selectTurnThinking` 决定 thinking，模型固定 `a.Model`，详见 [agent-loop.md](agent-loop.md) §2.5）。接线在 `cmd/dsc/main.go` 的 `applyRoutingConfig`：`[routing] auto_route`（或 `--auto-route`）开启；`escalation_model`（或 `--escalation-model`）留空时默认补成 `deepseek-v4-pro`——否则 `routeTurn` 会因 `EscalationModel` 为空而静默 no-op。

### 2.2 信号全列表（`routing.Classify`，按优先级序）

`Classify(s, cfg, prev)` 是纯启发式、零额外 LLM 调用的函数，四个分支按序短路：

1. **`sticky`** —— 上一步已在 pro 且 `prev.StickyLeft > 0`：留在 pro + thinking + `max` effort，`StickyLeft` 减一。目的是防 flash↔pro 抖动（一个难问题后面往往跟着难的追问）。`StickyTurns` 在 `routeTurn` 调用点硬编码为 `2`。
2. **`repair_errors`** —— `Signals.RepairErrorsLastTurn >= 3`：升 pro + `max` 并重新武装粘滞。**诚实标注**：`runStep` 当前在唯一调用点传的是字面量 `0`，所以这个信号目前只由 `internal/routing` 的单测驱动；生产路径上「修复错误触发升级」实际走的是 §2.3 的 turn 内通道（阈值同为 3）。给 `Classify` 喂真实修复计数是一个已留好接口、尚未接线的扩展点。
3. **`hard_reasoning`** —— 用户文本超过 240 字节，**或**小写后包含 `hardWords` 之一：`why`、`design`、`architect`、`refactor`、`debug`、`prove`、`root cause`、`race`、`deadlock`、`redesign`、`trade-off`、`tradeoff`（`classifier.go` 包级变量）。命中即 pro + thinking + `max` + 重新武装粘滞。
...
### 2.3 turn 内升级（第二通道）：`escalationTrigger`

`routeTurn` 是「事前预测」，第二通道是「事后补救」：flash 这一轮已经流完、`repairToolCalls` 修完之后、消息提交之前（`runStep` 中段），`escalationEnabled()`（`EscalationModel` 非空且不等于当前模型——pro 主循环永不向自己升级）成立时检查 `escalationTrigger(text, repairErrors)`：

- **`marker`** —— 模型自我声明：回复的首个非空行**整行**恰为 `<<<NEEDS_PRO: 原因>>>`（或无原因的 `<<<NEEDS_PRO>>>`），由 `needsProMarker` 解析；行内引用该 token（比如回显日志）不会误触发。marker 优先于阈值，便于归因。
...
3. **事件与归账**——发布 `EventEscalated{Trigger, FromModel, ToModel, Reason}`（trace 记录在 `trace.go`，ACP 转发在 `internal/acp/adapter.go`）；`streamWithReissue` 用同一请求体仅改 `Model` 重发；`respModel` 改为 `EscalationModel`，持久化 / 成本 / trace 全部记到真正产出该 turn 的模型头上。flash 那轮的输出整体丢弃，transcript 里只有 pro 版本，不会出现重复 turn。

一个容易踩的现状：教模型使用 marker 的系统提示契约 `escalationContract` / `Agent.EnableEscalation`（注入点在 `prompt.DynamicContextBoundary` 之前，进前缀指纹、对固定目标模型字节稳定）**在当前 `cmd/dsc` 接线中没有调用方**——`applyRoutingConfig` 直接赋值 `EscalationModel`，不注入契约。也就是说 marker 通道依赖模型自发输出该 token，或由自定义组装显式调用 `EnableEscalation`（注意其 `PromptBuilder` 场景的 no-op 守卫，注释解释了原因：builder 每 turn 重建 `a.System` 会静默抹掉注入并造成指纹漂移）。

### 2.4 澄清前置闸：花 pro 的钱之前先确认值不值

路由的省钱兄弟：`Agent.Run` 在进入 `agentLoop` 之前（`agent.go`），若 `shouldClarify(userPrompt)` 成立，直接发布一句 `Before I start: …` 并以 `StopModelDone` 返回——**零模型调用**。判定是 `routing.NeedsClarification`（`internal/routing/clarify.go`）：空 prompt、整句命中 `vaguePhrases`（`fix it` / `make it better` / `improve this` / `do the thing` / `make it work` / `clean it up` / `optimize it`），或 ≤3 个词且不含 `/`、`.`（没有路径样 token）。设计动机写在函数注释里：对模糊 prompt 猜错一发 pro/max turn，比先问一句贵得多。开关 `[clarify] auto_clarify` / `--auto-clarify`，默认关闭。
...
- **`bash`** → `permissions.IsDestructiveBash`（`internal/permissions/destructive.go`）：`tools.ClassifyBash(command) == BashDestructive`，或命中 `[duet] extra_destructive_patterns` 的用户自定义正则。`ClassifyBash`（`internal/tools/bash_validate.go`）把 `;` / `|` / `&&` / `||` 链拆段、取最严段定级；destructive 动词面包括 `rm`/`rmdir`/`mv`、`cp -f`、`sed -i`、`chmod`/`chown` 系、`kill`/`pkill`/`killall`、`dd`、`mkfs`/`mount`/`umount`、关机系、`docker rm/rmi/push`、`kubectl delete`、`npm|pnpm|yarn publish`、`terraform apply|destroy`、`curl --upload-file`，以及 git 的 `push --force/-f/--force-with-lease`、`reset --hard`、`checkout -- .`、`clean -f`——完整分级表的单一事实源是 [../reference/bash-validation.md](../reference/bash-validation.md)。注意：**普通 `git push`（无 force）在 `ClassifyBash` 中是 `BashSafe`**，不触发 Duet；`permissions.DestructivePatterns` 那张含 `git push` 的正则表按其注释只是历史存档与 extra-patterns 机制的参照，活跃判定以 `ClassifyBash` 为准。
- **`write_file` / `edit_file`** → `permissions.IsDestructiveToolCall` → `IsDestructivePath`：目标在 cwd 之外（相对路径以 `..` 开头）、路径任一段为 `.git`、或文件名命中 `[permissions] secret_path_patterns`。args 解析失败按 destructive 处理（fail-closed）。**其余所有工具一律不触发 Duet**（`IsDestructiveToolCall` 的 default 分支返回 `false`）。
...
1. **自跳过**——`modelFn()` 返回的当前主循环模型已是 `deepseek-v4-pro` 时直接 `continue`：pro 给自己当裁判没有意义。这与 `escalationEnabled` 的「已在目标模型上不升级」守卫互为镜像（两处注释互相引用）。
2. **非 destructive** → `allow`，零额外开销。
3. **destructive** → `buildDuetPrompt` 组裁决提示：transcript 尾部至多 4000 字节（`transcriptFn` 闭包提供）+ 工具名 + 原始 args + approve/block 准则，要求只回 `{"approve": bool, "reasoning": "…"}`。
...
跑法：`go test ./internal/routing/... ./internal/agent/... ./internal/hooks/... ./internal/permissions/... ./internal/tools/...`，全程不碰网络。
...
1. **`internal/routing/classifier.go`**——给 `Signals` 加字段（如 `ToolErrorsLastTurn int`），在 `Classify` 里插入判定分支。想清楚**优先级位置**（放在 sticky 之后、hard 之前还是之后决定了它能否被粘滞掩盖）、是否**重新武装** `StickyLeft`、以及给 `Reason` 起一个新的稳定字符串（trace 和测试都会断言它）。
...
5. **想清楚通道归属**——信号若来自「本步已经发生的事实」（模型输出、修复结果），考虑放进 `escalationTrigger`（turn 内重发）而不是 `Classify`（turn 前预测）；前者多花一轮 flash 的钱但判定更准，后者免费但只能看 turn 前可见的信号。
6. **缓存红线自查**——新信号的判定与生效都不得改前缀字节。若你发现自己想往系统提示里塞东西，请先读 [prefix-cache.md](prefix-cache.md) 的 golden 守卫一节，并参考 `escalationContract` 的注入纪律（边界之前、字节稳定、PromptBuilder 场景禁注入）。
...
三个数字都有明确的家：`StickyTurns` 在 `routeTurn` 调用点硬编码 `2`（`agent.go`，想配置化需先过 `routing.Config` 透传）；`escalationRepairThreshold` 是 `agent.go` 包级常量 `3`；hard 文本的长度阈值 `240` 与 `hardWords` 词表在 `classifier.go`。改任何一个，对应更新 `classifier_test.go` / `loop_escalation_test.go` 里写死的期望值——它们故意钉死数值，防止「顺手调参」绕过 review。
