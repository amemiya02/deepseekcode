...
| T5.4 | ✅ done | Diff auto-detection + golden-per-width tests. New conservative `looksLikeDiff` scanner (requires a `diff --git ` line or a `@@ … @@` hunk header, so shell output / source with +/- isn't misclassified) routes raw diffs — e.g. `git diff` run via bash, previously lexed as shell — through the diff lexer regardless of which tool produced them. Golden fixtures pin tool-call/result/reasoning rendering across narrow/default/wide widths (40/80/120) under `testdata/render/`, ANSI-stripped so they capture layout not color (regenerate with `UPDATE_GOLDEN=1`). Generating them surfaced a real latent bug the golden now locks: `chromaFormatter` rendered each chroma token via `lipgloss.Render`, which block-pads multi-line token values — and chroma emits whole lines (often with a trailing `\n`) as one token, so every highlighted line injected trailing spaces that surfaced as misaligned leading whitespace on the next line (catastrophic for diffs, subtle for shell). Fixed by splitting each token on `\n` and rendering line-segments individually. Fingerprint-safe (TUI only); golden tripwire unmoved. **T5 track complete.** |
| T7.4 | ✅ done | Documentation truthing (T7 track, done first per the design-survey order — docs-only, lowest risk, and a prerequisite for T7.2's doc-linkage test). New `docs/MODEL_COMPATIBILITY.md` documents the DeepSeek-specific wire facts (thinking-struct not bool, cache-stable `MarshalCacheStable` + the 50× discount and golden tripwires, finish-reason override, `SanitizeForDeepSeek` message-shape rules, two-tier 45s/20s stream timeouts for reasoner cold start, 1M window + 800k auto-compact, no-SDK/no-CGO build constraints, pricing). `docs/PARITY.md` gains an honest "intentionally shallow" section written as **prose/bullets, never a pipe-table** — `TestParityConsistency.parsePARITYNames` treats the first column of every `|`-row as a scenario name, so a table there would inject phantom scenarios and break CI (verified; the section explicitly documents this constraint). `MODEL_COMPATIBILITY.md` linked from both READMEs' Project Files / 项目文件 sections in one commit (bilingual ## parity preserved at 14/14). Zero Go source touched; golden tripwire + `TestParityConsistency` green. |
| T7.2 | ✅ done | `dsc doctor` + config diagnostics + doc-linkage test. (1) Builtin-hook name cross-check: new `internal/hooks.KnownBuiltinNames`/`IsKnownBuiltin` (authoritative set = `{"duet"}`); `config.ValidateStrict` now rejects an unknown builtin name (e.g. `"duett"`) with a fix hint, instead of letting it pass validation and silently fail-open at runtime. `config`→`hooks` verified cycle-free. (2) New `checkAgentDefs` doctor check (testable `agentDefsResult` core): reports defs loaded, flags cwd `.md` files silently skipped by the 64 KiB cap (compared via `AgentDef.Path` prefix so home-dir defs/name-shadowing don't false-positive), and warns on out-of-range `temperature`/`top_p`. (3) Doc-linkage: discovered `TestParityConsistency` ALREADY enforces PARITY.md↔manifest (so the survey's proposed `parity_linkage_test.go` would be redundant) — instead added `TestREADMEDocLinksResolve`, a non-redundant test asserting every repo-relative `.md` link in both READMEs resolves (guards the T7.4 `MODEL_COMPATIBILITY.md` link and future doc rot). Dropped an unreachable `MaxSteps<0` check (the parser only sets `MaxSteps` when `n>0`). Golden tripwire unmoved; full suite + vet green. |
| T7.1 | ✅ done | Apply agent-def frontmatter at spawn. The five parse-but-ignore fields now take effect: **MaxSteps** replaces the child's `MaxSteps(50)` StopCondition (step cap only — deliberately NOT also `MaxToolCalls`, which the survey flagged as a conflating bug that would make the step-count test wrong); **Temperature/TopP** flow through new nil-default `Agent.Temperature/Agent.TopP` fields into the `llm.Request` (TopP newly added to `Request` + `MarshalCacheStable` — `omitempty` + nil on the main loop so the golden bytes/fingerprint are byte-identical, verified); **PermissionRuleset** maps recognized names → `Mode` via new `permissions.ModeFromRuleset`, fed through `DeriveChild` so it can only RESTRICT (clampMode prevents escalation), plan mode taking precedence; **DefaultAgent** surfaces an `EmitInfo` notice (undefined spawn semantics — deliberately not the survey's speculative fallback). Tests assert temperature/top_p reach the wire body, the MaxSteps cap ends the loop at 3 requests (vs default 50), and ModeFromRuleset maps + cannot escalate. `def.Prompt` handling left untouched (already worked). Golden tripwire unmoved; full suite + vet green. |
| T6.1 | ✅ done | Single trace-schema source of truth + `schema_version`. New `internal/traceschema.Record` is the one canonical agent-trace record; the emitter (`internal/agent/trace.go`), the inspector (`internal/traceinspect`), and the benchmark harness's agent-trace parser (`bench/cmd/benchrunner` `traceLine`) all become **type aliases** of it — so a field rename is now a compile error in every consumer rather than a silent reader breakage. `traceschema.Version` (=1) is stamped at the single `encode` chokepoint so every emitted record carries `schema_version` regardless of emit site (test `TestTraceRecordsCarrySchemaVersion`). **Roadmap-cite correction:** `bench/cmd/benchrunner/main.go:107` `TraceRecord` is the harness's own run-summary WRITER (success/error/duration — a deliberately distinct schema), not the agent-trace reader; the real triplicate was `traceRecord`/`traceinspect.record`/`traceLine`. `bench/` is in the main module (only `./go.mod`), so it imports the shared `internal/` type directly — no codegen-from-fixture needed. Trace JSONL is not cache-protected; golden tripwire unmoved, bench golden gate (static fixtures) intact, full suite + vet green. |
| T6.2 | ✅ done | Reconcile `eventschema` — **route (a) RETIRE**. Evidence gathered first: the package had **zero importers** (only its own `schema.go`+`schema_test.go`; the `internal/agent` hits were stale comments, not imports), and its 16 constants matched almost none of the 11 actually-emitted trace strings — `model.turn.*`, `tool.*`, `subagent.*`, `session.compacted`, `prefix.epoch.*`, `prefix.pending_change`, `prefix.drift.blocked` were pure fiction (never emitted). Route (b)-canonicalize was rejected: it either renames emitted wire strings (breaks the `bench/golden-traces` fixtures) or is a high-touch 3-surface rewiring to wire a registry the consumers don't need (traceinspect/benchrunner string-match fine). So: deleted `internal/eventschema/`, removed the "canonical event-name constants … downstream consumers" section from `CLAUDE.md`, and cleaned the stale `eventschema.*` comment references in `trace.go`/`events.go`/`docs`. Build/full-suite/vet green after deletion (confirming zero real importers); golden tripwire unmoved. |
| T6.4 | ✅ done | Trace inspect surfaces lifecycle + realized-vs-projected cost, and re-grades the golden fixtures offline. `traceinspect` previously parsed only `usage`/`agent.done`, so its summary looked clean even on a trace the Cache Reliability gate would fail — it now counts `compaction`/`drift.blocked`/`pending_change` records and renders them. The budget gate's projected turn cost (`EventBudget.ProjectedCNY`, previously dropped) is now emitted on `budget.*` records via a new `traceschema.Record.ProjectedCNY` field; the inspector sums it and renders `cost realized ¥X vs projected ¥Y (Δ …)` (Δ = conservative all-miss projection − cache-discounted realized = the cache savings). `TestInspectRegradesGoldenFixtures` re-grades all four `bench/golden-traces/*.jsonl` offline (no loop, no tokens) — the offline-re-grade interpretation of "replay"; re-running the live loop was out of scope as the lower-value reading. Trace JSONL is not cache-protected (new `projected_cny` is omitempty/nil elsewhere); golden tripwire unmoved, full suite + vet green. **T6 track complete (T6.1–T6.4).** |
| T3.5 | ✅ done | `/undo` reconciles the transcript. `/undo` reverted files only; `a.Messages` and disk still claimed the reverted turn succeeded. New `agent.ReconcileUndo(n)` truncates `a.Messages` (and persisted rows via the optional `MessageTruncator`) back to the boundary the n-th-from-last **snapshotted** step started from — `/undo` counts SNAPSHOTS (sparse), matching the file revert, via a new `StepRecord.Snapshotted` flag; `StepRecord.MessageCount` records the pre-turn boundary. Refuses across a compaction (`compactionFloor`, raised at both — and only the two — message-swap sites) since compaction renumbers messages. Fingerprint byte-identical (System/Tools untouched), asserted in test. **Hardened by a 3-lens adversarial workflow that found 3 real HIGH defects:** (1) on a resumed session `a.steps` is empty so the reconcile silently no-op'd while files reverted → `applyUndo` now WARNS when files reverted but nothing rewound; (2)+(3) the in-memory boundary equals the persisted disk idx ONLY for a fresh, never-resumed/branched/repaired session — so `ReconcileUndo` now verifies `len(a.Messages) == PersistedMessageCount()` and refuses (mutating nothing) when they diverge, instead of deleting wrong disk rows. Plus a LOW: `applyUndo` now reads `a.running` under `runMu` (matching the ctrl+c handler). Tests cover single/double/clamp/no-op/compaction-refusal/misalignment-refusal + the Store truncate; full suite + `-race` green, golden unmoved. |
| T7.3 | ✅ done | Skill hot-reload + cheaper read-only subagents + admin policy floor. (1) **`/reload-skills`** (`agent.ReloadSkills`): re-scans the skill dirs, swaps the canonical store in place (`skills.Store.ReplaceFrom` — the SAME pointer `a.Skills` and the `skill_read` dispatcher share, so the capability set and on-demand bodies refresh together), rebuilds `a.System` from the `PromptBuilder`, and mints a NEW prefix epoch via the existing `SwitchEpoch` — **only when the model-visible prefix actually moved** (a body edit moves the body-derived `version_hash` in the directory; a no-op reload mints nothing, so no wasted cache miss). The one sanctioned exception to `LoadScan`'s session-start-only rule, documented there. (2) **Cheaper read-only subagents:** `model:` already selects a child's tier (asserted reaching the wire); new `omit_project_context:` frontmatter trims the inherited parent's per-turn project-context block (cwd/date/git status/diff) from a read-only/explore child via a pure `childSystem` helper — honestly named (it omits the project *context* block, not skills/instructions/memory, which sit before the boundary). (3) **`.deepseek/requirements.toml` admin floor:** an opt-in, repo-committed ceiling (`max_mode`, `require_sandbox`) enforced ABOVE the `--yolo`/`--read-only` flags on BOTH startup paths — a launch exceeding it is REFUSED, not clamped, so a repo can forbid `dsc --yolo`. **Hardened by a 3-lens adversarial pass that found 3 real HIGH defects:** (a) a still-live async subagent's `skill_read` could race `ReplaceFrom` on the shared store (`a.running` covers only the main loop) → reload now also refuses while `HasActiveBackgroundWork()`; (b) a nil-`PromptBuilder` path swapped the store invisibly (no prefix move/epoch/receipt) → now refused; (c) `require_sandbox` checked config intent, not the effective sandbox, so it honest-washed an unsandboxed launch on unsupported platforms (Detect→noop) → now checks `Available() && Name()!="noop"`. Plus MEDIUM/LOW: documented the `ask-all < default` floor ordering (+ test), and the refusal now names the requested mode. Golden tripwire unmoved; full suite + vet + `-race` (agent/tui/skills) green. **T7 track complete. Roadmap complete except T3.4 (a deliberate security non-goal — sandbox stays opt-in).** |
...
pending an explicit decision. **T3.5** (/undo transcript reconcile), **T6**
(T6.1/T6.2/T6.4), and all of **T7** (T7.1–T7.4) have since landed.

**Status: 28 of 29 stages complete.** The only remaining stage is **T3.4**,
deferred by explicit decision as a security non-goal (the OS sandbox stays
opt-in). Every other track — T1, T2, T4, T5, T6, T7 — is fully done and
verified, golden cache tripwires unmoved throughout.
