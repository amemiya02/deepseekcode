...
func TestProjectedTurnCostCNYUsesMissPricingConservatively(t *testing.T) {
	...
	got := ProjectedTurnCostCNY("deepseek-v4-flash", req, defaultCharsPerToken, 0, 0)
	if got <= 0 {
		t.Fatalf("ProjectedTurnCostCNY = %v, want positive cost", got)
	}

	inputTokens := EstimateInputTokens(req.Messages, defaultCharsPerToken)
...
func TestProjectedTurnCostCNYUnknownModelReturnsZero(t *testing.T) {
	...
	if got := ProjectedTurnCostCNY("unknown-model", req, defaultCharsPerToken, 0, 0); got != 0 {
...
func TestProjectedTurnCostCNYHitRateBlend(t *testing.T) {
	...
	if got := ProjectedTurnCostCNY(model, req, defaultCharsPerToken, 0, 0); got != allMiss {
	...
	if got := ProjectedTurnCostCNY(model, req, defaultCharsPerToken, 1.0, 0); got != allHit {
	...
	half := ProjectedTurnCostCNY(model, req, defaultCharsPerToken, 0.5, 0)
	...
	if got := ProjectedTurnCostCNY(model, req, defaultCharsPerToken, 1.5, 0); got != allHit {
		t.Errorf("hitRate 1.5 should clamp to 1.0: got %.12f, want %.12f", got, allHit)
	}
	if got := ProjectedTurnCostCNY(model, req, defaultCharsPerToken, -0.5, 0); got != allMiss {
